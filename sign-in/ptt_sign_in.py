...
# 固定签到地址
sign_in_url = "https://www.pttime.org/attendance.php?type=list"

# 定义常量
COOKIE_ENV = 'PTT_COOKIE'
...
async def check_response_status(response):
    ...
    if response.status_code == 200:
        if "<title>PTT :: 登录 - Powered by NexusPHP</title>" in response.text:
            logging.info("签到失败：登录已经过期")
            return "签到失败：登录已经过期"
        elif "<title>PTT :: 签到 - Powered by NexusPHP</title>" in response.text:
            logging.info("签到失败")
            return "签到失败"
        else:
